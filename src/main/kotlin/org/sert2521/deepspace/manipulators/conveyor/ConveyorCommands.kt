...
suspend fun Conveyor.run(invert: Boolean = false, override: Boolean = false) = use(this, name = "Run Conveyor") {
    periodic {
...
suspend fun Conveyor.runTimed(time: Double, invert: Boolean = false) = use(this, name = "Timed Run Conveyor") {
    timer(time) {
        Conveyor.spin(CONVEYOR_SPEED * if (invert) -1 else 1)
